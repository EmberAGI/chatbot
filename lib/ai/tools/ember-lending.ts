...
export const getTools = async (): Promise<{ [key: string]: CoreTool }> => {
  ...
  if (agentIdFromCookie && agentIdFromCookie.value === 'ember-lp') {
    serverUrl = process.env.MCP_SERVER_URL || 'http://localhost:3012/sse';
  }

  if (agentIdFromCookie && agentIdFromCookie.value === 'all') {
    const serverUrls = [
      'http://173.230.139.151:3010/sse',
      'http://173.230.139.151:3011/sse',
    ];
    ...
    try {
      toolsResponse = await Promise.all(
        mcpClients.map((client) => client.listTools()),
      );
    ...
    const mergedToolsResponse = {
      tools: [...toolsResponse[0].tools, ...toolsResponse[1].tools],
    };

    console.log(mergedToolsResponse);
    // Use reduce to create an object mapping tool names to AI tools

    const toolObject = mergedToolsResponse.tools.reduce(
      (acc, mcptool, index) => {
        ...
        const aiTool = tool({
          ...
          execute: async (args) => {
            console.log(
              'Executing tool:',
              `${agentIdFromCookie.value}[${index}] - ${mcptool.name}`,
            );
